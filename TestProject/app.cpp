...
void RenderGraphCanvas() {
    ...
    ImGui::SetNextWindowSizeConstraints(ImVec2(FLT_MIN, 0), ImVec2(FLT_MAX, ImGui::GetIO().DisplaySize.y - controlsHeight));

    ImGui::Begin("Graph Canvas", nullptr, ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoCollapse);
    ...
    for (int i = 0; i < nodes.size(); ++i) {
        ...
        ImGui::GetWindowDrawList()->AddCircleFilled(pos, 15.0f, node.color);

        // Draw text inside the node indicating its index
        char buffer[32];
    ...
    for (const Node& node : nodes) {
        for (int connectedNodeIndex : node.connectedNodes) {
            const ImVec2& start = nodes[connectedNodeIndex].pos;
            const ImVec2& end = node.pos;
...
void RenderControls() {
    ...
    ImGui::SetNextWindowSize(ImVec2(controlWindowWidth, controlWindowHeight), ImGuiCond_Always);
    ...
    if (ImGui::Button("Add Node")) {
        ImU32 nodeColor = IM_COL32(255, 255, 0, 255);
        nodes.push_back({ ImVec2(100.0f, 100.0f), {}, nodeColor });
    }

    if (ImGui::Button("Add Link")) {
        // Select a node for creating a link
        if (nodes.size() > 1) {
            selectedNode = nodes.size() - 1; // Select the last added node
        }
    }
...
int main(int, char**)
{
    ...
    WNDCLASSEXW wc = { sizeof(wc), CS_CLASSDC, WndProc, 0L, 0L, GetModuleHandle(nullptr), nullptr, nullptr, nullptr, nullptr, L"Graph/Network Visualizer", nullptr };
    ::RegisterClassExW(&wc);
